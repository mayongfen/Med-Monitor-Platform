...
export function DeviceView() {
  ...
  const totalValue = DEVICES_LIFECYCLE.reduce((s, d) => s + depreciation(d).netValue, 0)
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm">
              <Plus className="size-4" /> 设备入库
            </Button>
          </div>
        </div>
        <p className="mb-3 text-xs text-muted-foreground">
          管理总数 = 在用 {stats.in_use ?? 0} + 备用 {stats.standby ?? 0} + 故障 {stats.fault ?? 0} + 报废 {stats.scrapped ?? 0} · 当前资产净值合计 ¥{totalValue.toLocaleString()}
        </p>
              ...
              {DEVICES_LIFECYCLE.map((d) => {
                ...
                return (
                  <tr key={d.id} className="border-b border-border/60 last:border-0 hover:bg-muted/50">
                    ...
                    <td className="whitespace-nowrap px-3 py-2.5 font-medium text-foreground">{d.model}</td>
                    ...
                    <td className="px-3 py-2.5 text-muted-foreground">¥{d.originValue.toLocaleString()}</td>
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex gap-2 text-xs">
                        ...
                        <button className="text-destructive hover:underline">报废</button>
                      </div>
                    </td>
                  </tr>
                )
        ...
        <p className="mt-3 text-xs text-muted-foreground">
          设备资产按直线折旧法（年限 3 年）计提；报废条件：累计使用超寿命、维修成本超原值 50%、关键部件损坏且无替换件。
        </p>
      </div>
    </div>
  )
